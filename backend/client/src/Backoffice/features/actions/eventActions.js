...
export const createEventAction = (nameEvent, startDateEvent, endDateEvent, descriptionEvent, urlEvent, location) => async (dispatch, getState) => {
    try {
        ...
        const data = await axios.post(process.env.REACT_APP_URI_SERVER + "/events/createEvent", { nameEvent, startDateEvent, endDateEvent, descriptionEvent, location, urlEvent }, config);
        console.log(data);
        dispatch({
...
export const deleteEventAction = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.delete(process.env.REACT_APP_URI_SERVER + `/events/delete/${id}`, config);
...
export const updateEventAction = (id, nameEvent, startDateEvent, endDateEvent, descriptionEvent, urlEvent, location) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(process.env.REACT_APP_URI_SERVER + `/events/update/${id}`, { nameEvent, startDateEvent, endDateEvent, descriptionEvent, urlEvent, location }, config);
...
export const getEventsByFilter = (arg) => async (dispatch) => {
    try {
        const response = await axios.post(process.env.REACT_APP_URI_SERVER + "/events/search", arg);
...
export const commentEvent = (value, id) => async (dispatch) => {
    try {
        console.log(id);
        const data = await axios.post(process.env.REACT_APP_URI_SERVER + `/events/commentEvent/${id}/`, value);
        console.log(data);
...
export const deleteCommentAction = (_id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.delete(process.env.REACT_APP_URI_SERVER + `/commentEvent/delete/${_id}`, config);
...
export const updateCommentAction = (id, comment) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(process.env.REACT_APP_URI_SERVER + `/commentEvent/update/${id}`, { comment }, config);
...
export const likeEvent = (id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.patch(process.env.REACT_APP_URI_SERVER + `/events/${id}/likeEvent`,);
