...
export default class CompaignList extends Component {
  ...
  createAndDownloadPdf = () => {
    axios.post(process.env.REACT_APP_URI_SERVER + '/compaigns/addPDF', this.state)
  ...
  componentDidMount() {
    axios.get(process.env.REACT_APP_URI_SERVER + '/compaigns/')
      .then(response => {
  ...
  deleteCompaign(id) {
    axios.delete(process.env.REACT_APP_URI_SERVER + '/compaigns/' + id)
  ...
  searchHandel = (event) => {
    ...
    axios.get(process.env.REACT_APP_URI_SERVER + '/compaigns/search/' + key)
      .then(response => {
        this.setState({ compaigns: response.data })
      })
      .catch((error) => {
