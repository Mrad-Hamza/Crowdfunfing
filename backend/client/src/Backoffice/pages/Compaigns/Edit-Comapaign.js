...
export default class CompaignEdit extends Component {
  ...
  componentDidMount() {
    axios.get(process.env.REACT_APP_URI_SERVER + '/compaigns/' + this.props.match.params.id)
      .then(response => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    axios.put(process.env.REACT_APP_URI_SERVER + '/compaigns/update/' + this.props.match.params.id, compaign)
