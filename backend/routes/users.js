...
router.route("/addUserImage").put(upload.single("image"), (req, res) => {
    ...
    User.findByIdAndUpdate(req.body.data).then((user) => {
        user.img = image;
        user.save()
...
router.post("/googlelogin", (req, res) => {
    ...
    client.verifyIdToken({ idToken, audience: "98128393533-fb736bc4b2637vn8t1028bcf0e6mv0lj.apps.googleusercontent.com" }).then((response) => {
...
router.route("/affectImage/:idU/:idM").put((req, res) => {
    User.findByIdAndUpdate(req.params.idU).then((user) => {
