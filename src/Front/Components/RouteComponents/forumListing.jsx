...
class ForumList extends Component {
    ...
    deleteForum(id) {
        axios.delete("http://localhost:5000/forums/" + id).then((response) => {
    ...
    showComment(id) {
        axios.get("http://localhost:5000/forums/com/" + id).then((res) => console.log(res.data));
    }

    searchHandel = (event) => {
        let key = event.target.value;
        axios
            .get("http://localhost:5000/forums/search/" + key)
            .then((response) => {
                this.setState({ forums: response.data });
            })
            .catch((error) => {
